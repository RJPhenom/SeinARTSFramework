 ...
 * @file    SeinNavigationDefaultAStar.cpp
 */

#include "Default/SeinNavigationDefaultAStar.h"
#include "Default/SeinNavigationDefaultAStarAsset.h"
...
DEFINE_LOG_CATEGORY_STATIC(LogSeinNavigationDefaultAStar, Log, All);

// ============================================================================
// Asset class
// ============================================================================

TSubclassOf<USeinNavigationAsset> USeinNavigationDefaultAStar::GetAssetClass() const
{
	return USeinNavigationDefaultAStarAsset::StaticClass();
}
...
bool USeinNavigationDefaultAStar::BeginBake(UWorld* World)
{
	if (!World)
	{
		UE_LOG(LogSeinNavigationDefaultAStar, Warning, TEXT("BeginBake: null world"));
		return false;
	}
	if (bBaking)
	{
		UE_LOG(LogSeinNavigationDefaultAStar, Warning, TEXT("BeginBake: already baking"));
		return false;
	}
	...
	USeinNavigationDefaultAStarAsset* NewAsset = nullptr;
	const bool bOk = DoSyncBake(World, NewAsset);
	if (!bOk || !NewAsset)
	{
		UE_LOG(LogSeinNavigationDefaultAStar, Warning, TEXT("BeginBake: failed"));
		return false;
	}
	...
	UE_LOG(LogSeinNavigationDefaultAStar, Log, TEXT("Bake complete: %dx%d cells, CellSize=%s"),
		NewAsset->Width, NewAsset->Height, *NewAsset->CellSize.ToString());
	return true;
}

bool USeinNavigationDefaultAStar::DoSyncBake(UWorld* World, USeinNavigationDefaultAStarAsset*& OutAsset)
{
	OutAsset = nullptr;
	...
	{
		UE_LOG(LogSeinNavigationDefaultAStar, Warning, TEXT("DoSyncBake: no NavVolumes in world"));
		return false;
	}
...
#if WITH_EDITOR
	FScopedSlowTask Task(GridW * GridH, NSLOCTEXT("SeinNavigationDefaultAStar", "Baking", "Baking SeinARTS A* Nav..."));
...
#else
	OutAsset = NewObject<USeinNavigationDefaultAStarAsset>(GetTransientPackage());
#endif
	...
	//      Vehicles, infantry, and any other unit that runs movement should
	...
	for (TActorIterator<ASeinActor> It(World); It; ++It)
	{
		...
		// archetype is movement-driven, so its at-bake pose is irrelevant.
	...
	UE_LOG(LogSeinNavigationDefaultAStar, Log,
	...
	for (int32 Y = 0; Y < GridH; ++Y)
	{
		for (int32 X = 0; X < GridW; ++X)
		{
			if (bCancelRequested)
			{
				UE_LOG(LogSeinNavigationDefaultAStar, Warning, TEXT("Bake cancelled by user"));
				OutAsset = nullptr;
				return false;
			}
	...
	// ========================================================================
	{
		...
		UE_LOG(LogSeinNavigationDefaultAStar, Log, TEXT("Bake connectivity: edges=%d blocked_edges=%d FallbackStep=%.1f"),
	...
	// ========================================================================
	{
		...
		// ground, or whether a stray connection bit merged them.
		{
			...
			UE_LOG(LogSeinNavigationDefaultAStar, Log, TEXT("Bake component sizes: %s"), *Sizes);
		}
	...
	{
		UE_LOG(LogSeinNavigationDefaultAStar, Warning, TEXT("Bake: failed to save asset to disk"));
...
#if WITH_EDITOR
USeinNavigationDefaultAStarAsset* USeinNavigationDefaultAStar::CreateOrLoadAsset(UWorld* World, const FString& AssetName) const
{
	...
	USeinNavigationDefaultAStarAsset* Existing = FindObject<USeinNavigationDefaultAStarAsset>(Package, *AssetName);
	if (Existing) return Existing;

	USeinNavigationDefaultAStarAsset* Asset = NewObject<USeinNavigationDefaultAStarAsset>(
		Package, USeinNavigationDefaultAStarAsset::StaticClass(), FName(*AssetName),
		RF_Public | RF_Standalone);
	FAssetRegistryModule::AssetCreated(Asset);
	return Asset;
}

bool USeinNavigationDefaultAStar::SaveAssetToDisk(USeinNavigationDefaultAStarAsset* Asset) const
{
...
void USeinNavigationDefaultAStar::LoadFromAsset(USeinNavigationAsset* Asset)
{
	Super::LoadFromAsset(Asset);

	if (USeinNavigationDefaultAStarAsset* AStarAsset = Cast<USeinNavigationDefaultAStarAsset>(Asset))
	{
...
void USeinNavigationDefaultAStar::ApplyAssetData(const USeinNavigationDefaultAStarAsset* Asset)
{
	Width = Asset->Width;
	Height = Asset->Height;
...
bool USeinNavigationDefaultAStar::WorldToGrid(const FFixedVector& WorldPos, int32& OutX, int32& OutY) const
{
...
FFixedVector USeinNavigationDefaultAStar::GridToWorld(int32 X, int32 Y) const
{
...
bool USeinNavigationDefaultAStar::IsPassable(const FFixedVector& WorldPos) const
{
	int32 X, Y;
...
void USeinNavigationDefaultAStar::SetDynamicBlockers(const TArray<FSeinDynamicBlocker>& InBlockers)
{
...
void USeinNavigationDefaultAStar::BuildDynamicBlockedOverlay(FSeinEntityHandle Exclude, uint8 AgentNavLayerMask) const
{
	const int32 N = Width * Height;
...
bool USeinNavigationDefaultAStar::GetGroundHeightAt(const FFixedVector& WorldPos, FFixedPoint& OutZ) const
{
	int32 X, Y;
	if (!WorldToGrid(WorldPos, X, Y)) return false;
	...
	// and visibly pops onto the wall. Returning false here makes movement
...
bool USeinNavigationDefaultAStar::ProjectPointToNav(const FFixedVector& WorldPos, FFixedVector& OutProjected) const
{
...
TArray<FIntPoint> USeinNavigationDefaultAStar::AStarSearch(FIntPoint Start, FIntPoint End) const
{
...
bool USeinNavigationDefaultAStar::HasLineOfSight(int32 X0, int32 Y0, int32 X1, int32 Y1) const
{
...
void USeinNavigationDefaultAStar::BuildSmoothedPath(const TArray<FIntPoint>& CellPath, FSeinPath& OutPath) const
{
	OutPath.Clear();
...
bool USeinNavigationDefaultAStar::FindPath(const FSeinPathRequest& Request, FSeinPath& OutPath) const
{
	OutPath.Clear();
	if (!HasRuntimeData()) return false;
	...
	{
		UE_LOG(LogSeinNavigationDefaultAStar, Verbose, TEXT("FindPath: start out of bounds"));
		return false;
	}
	int32 EX, EY;
	if (!WorldToGrid(Request.End, EX, EY))
	{
		UE_LOG(LogSeinNavigationDefaultAStar, Verbose, TEXT("FindPath: end out of bounds"));
		return false;
	}
...
void USeinNavigationDefaultAStar::CollectDebugCellQuads(TArray<FVector>& OutCenters, TArray<FColor>& OutColors, float& OutHalfExtent) const
{
...
void USeinNavigationDefaultAStar::CollectDebugBlockerCells(
	TArray<FVector>& OutCenters,
	TArray<FColor>& OutColors,
	float& OutHalfExtent) const
{
#if UE_ENABLE_DEBUG_DRAWING
	UE_LOG(LogSeinNavigationDefaultAStar, Verbose,
...
void USeinNavigationDefaultAStar::CollectDebugPathCells(
