...
FFixedVector USeinLocomotion::ResolveLookAheadPoint(
...
FFixedPoint USeinLocomotion::StepSpeedToward(
...
FFixedPoint USeinLocomotion::KinematicArrivalSpeedCap(
...
FFixedVector USeinLocomotion::ResolveNavCollision(
	...
	USeinNavigation* Nav)
{
	...
	// ground-snap happens after this in the locomotion's own pass.
	...
	// Cornered / dead-ended. Hold position. Locomotion's existing logic
	// (turn-rate, avoidance bias, repath) drives the next attempt.
	return OldPos;
}

bool USeinLocomotion::IsOvershootArrival(
	const FFixedVector& AgentPos,
...
bool USeinLocomotion::ShouldAutoReverse(
	const FFixedVector& AgentPos,
	const FFixedQuaternion& Rotation,
...
bool USeinLocomotion::IsClusterArrival(
...
FFixedVector USeinLocomotion::ComputeAvoidanceVector(
	const FSeinLocomotionContext& Ctx,
	...
	FFixedPoint LookAheadSeconds)
{
	...
	// reference member of FSeinLocomotionContext stays mutable through a
	...
	// exact for non-strafing locomotions (everyone we currently ship).
	...
	// ~5 ticks (≈165ms at 30Hz). The locomotion further damps via
	...
	// bias the locomotion's path-following dominates and any residual is
	...
	// Per-call summary. Fires every tick a locomotion calls this — verbose
	// only, so off in release. Useful for "why isn't unit X avoiding?" —
	// shows neighbor count, raw bias, and the smoothed value the locomotion
	...
	// the function's return value. Locomotion subclasses always consume the
	// smoothed result.
	Bias = Smoothed;
	...
	//   - Cyan = current velocity (Forward × CurrentSpeed). Scaled 10× so it
	//     reads cleanly at typical RTS unit speeds (200-400 cm/s); without
	//     the scale a meter-long arrow at full speed gets lost on screen.
	...
	if (UE::SeinARTSNavigation::IsSteeringVectorsShowFlagOn())
	{
		if (UWorld* DebugWorld = Ctx.World->GetWorld())
		{
			const FVector Origin(SelfPos.X.ToFloat(), SelfPos.Y.ToFloat(), SelfPos.Z.ToFloat() + 50.0f);
			DrawDebugPoint(DebugWorld, Origin, 6.0f, FColor::Cyan, false, 0.0f);

			// Footprint ring — yellow circle at ground level showing the
			// unit's collision radius. Lets you eyeball "are these two units
			// actually overlapping their footprints?" against the
			// penetration-resolution behavior, and "is the perception/
			// engagement zone tuned right?" against the bias arrow length.
			// Drawn in the XY plane (YAxis = world X, ZAxis = world Y) — the
			// "ZAxis" arg is the circle plane's normal-mapped basis, NOT
			// world up.
			...
			// at 1:1 scale (arrow length = "where the unit will be in one
			// second at this velocity"). Sign of CurrentSpeed encodes
			// reverse, so reversing units render the arrow opposite their
			// facing.
			...
			if (FMath::Abs(SpeedF) > 1.0f)
			{
				...
				DrawDebugDirectionalArrow(DebugWorld, Origin, VelEnd,
					64.0f, FColor::Cyan, false, 0.0f, 0, 7.0f);
			}
			...
			if (BiasMag > 1.0f)
			{
				...
				DrawDebugDirectionalArrow(DebugWorld, Origin, End,
					64.0f, FColor::Magenta, false, 0.0f, 0, 7.0f);
			}
		}
	}
#endif
